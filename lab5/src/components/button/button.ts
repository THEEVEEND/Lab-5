...
import { changeHead, changeChest, changeLegs } from "../../store/actions";
...
export class changeButton extends HTMLElement {
    ...
    render() {
        ...
        button.addEventListener("click", () => {
            if (this.properties.type === buttonType.head) {
                dispatch(
                    changeHead(this.properties.id)
                )
            }

            if (this.properties.type === buttonType.chest) {
                dispatch(
                    changeChest(this.properties.id)
                )
            }

            if (this.properties.type === buttonType.legs) {
                dispatch(
                    changeLegs(this.properties.id)
                )
            }
        })
    }
